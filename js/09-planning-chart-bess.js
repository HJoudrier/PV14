function renderBessChart(planData, hoursExt) {
  ...
  if (ctxBess) {
    const pMax = Math.max(state.params.bessPowerKw * 1.15, 20);
    ...
    const powerMinLine = new Array(hoursExt.length).fill(planData.powerMinLimit);
    const powerMaxLine = new Array(hoursExt.length).fill(planData.powerMaxLimit);
    ...
    if (state.charts.planningBess) {
      ...
      chart.data.datasets[6].data = powerMinLine;
      chart.data.datasets[7].data = powerMaxLine;
        ...
        data: {
          ...
          datasets: [
            ...
            },
            {
              // Limite de puissance en charge.
              type: 'line',
              label: 'Limite puissance (décharge)',
              data: powerMaxLine,
              yAxisID: 'yPower',
              borderColor: '#64748b',
              borderWidth: 1.5,
